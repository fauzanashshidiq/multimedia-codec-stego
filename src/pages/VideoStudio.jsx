...
const VideoStudio = () => {
  ...
  const handleDecompressFile = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setDecompressInfo(`Ukuran File: ${(file.size / (1024 * 1024)).toFixed(2)} MB | Tipe: ${file.type}`);
    }
  };
  ...
  const handleEncodeAction = async () => {
    ...
    setProgress(0);
    try {
      const result = await encodeVideoStego(
        encodeFile,
        secretMessage,
        onProgress,
      );
    ...
    <div className="animate-fade-in" style={{ paddingBottom: "3rem" }}>
      <header style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ color: "var(--text-secondary)" }}>
          Powered by FFmpeg.wasm. Kompresi video dan sisipkan data ke dalam
          metadata secara langsung di browser Anda.
        </p>
      </header>
      ...
      >
          ...
          style={{
            background:
              activeTab === "compress"
                ? "var(--accent-primary)"
                : "transparent",
            color: activeTab === "compress" ? "#fff" : "var(--text-secondary)",
            border: "none",
            padding: "0.5rem 1rem",
          ...
          style={{
            background:
              activeTab === "decompress"
                ? "var(--accent-primary)"
                : "transparent",
            color: activeTab === "decompress" ? "#fff" : "var(--text-secondary)",
            border: "none",
            padding: "0.5rem 1rem",
            borderRadius: "var(--radius-md)",
            cursor: "pointer",
          ...
          style={{
            background:
              activeTab === "stego-encode"
                ...
                : "transparent",
            color:
              activeTab === "stego-encode" ? "#fff" : "var(--text-secondary)",
            border: "none",
            padding: "0.5rem 1rem",
            borderRadius: "var(--radius-md)",
            cursor: "pointer",
        ...
        <button
          ...
          style={{
            background:
              activeTab === "stego-decode" ? "#10b981" : "transparent",
            color:
              activeTab === "stego-decode" ? "#fff" : "var(--text-secondary)",
            border: "none",
            padding: "0.5rem 1rem",
            borderRadius: "var(--radius-md)",
            cursor: "pointer",
      ...
      {loading && (
        ...
        >
          <h3
            style={{ marginBottom: "0.5rem", color: "var(--accent-primary)" }}
          >
            Memproses FFmpeg...
          </h3>
          <div
        ...
        <div className="glass-card animate-fade-in">
          ...
          <button
            ...
            style={{
              ...
              borderRadius: "var(--radius-md)",
              cursor: !compressFile ? "not-allowed" : "pointer",
              opacity: !compressFile ? 0.5 : 1,
              fontWeight: "bold",
            }}
          >
            Mulai Kompresi
      ...
      {/* DECOMPRESS TAB */}
      {activeTab === "decompress" && !loading && (
        <div className="glass-card animate-fade-in">
          <h2 style={{ marginBottom: "1.5rem" }}>Dekompresi Video</h2>
          <p style={{ marginBottom: "1.5rem", color: "var(--text-secondary)", fontSize: "0.9rem" }}>
            Unggah video hasil kompresi untuk melihat ukurannya dan mendekode formatnya ke dalam memori agar bisa diputar.
          </p>
          
          <div style={{ marginBottom: "1.5rem" }}>
            <label style={{ display: "block", marginBottom: "0.5rem", color: "var(--text-secondary)" }}>Pilih Video Terkompresi</label>
            <input type="file" accept="video/*" onChange={handleDecompressFile} style={{ width: "100%", padding: "0.75rem", background: "var(--bg-secondary)", border: "1px solid var(--border-glass)", borderRadius: "var(--radius-sm)", color: "#fff" }} />
          </div>

          <button 
            onClick={handleDecompressAction} 
            disabled={!decompressFile}
            style={{ background: "var(--accent-primary)", color: "#fff", border: "none", padding: "0.75rem 1.5rem", borderRadius: "var(--radius-md)", cursor: !decompressFile ? "not-allowed" : "pointer", opacity: !decompressFile ? 0.5 : 1, fontWeight: "bold" }}>
            Mulai Dekompresi (Putar)
          </button>

          {decompressPreview && (
            <div style={{ marginTop: "2rem", paddingTop: "2rem", borderTop: "1px solid var(--border-glass)" }}>
              <h3 style={{ marginBottom: "0.5rem", color: "#10b981" }}>✅ Video Berhasil Didekompresi</h3>
              <p style={{ color: "var(--accent-primary)", marginBottom: "1rem", fontWeight: "bold" }}>{decompressInfo}</p>
              <video controls src={decompressPreview} style={{ width: "100%", maxWidth: "600px", borderRadius: "var(--radius-md)", marginBottom: "1rem" }} />
            </div>
          )}
        </div>
      )}
        ...
        <div className="glass-card animate-fade-in">
          ...
          <button
            ...
            style={{
              ...
              padding: "0.75rem 1.5rem",
              borderRadius: "var(--radius-md)",
              cursor: !encodeFile || !secretMessage ? "not-allowed" : "pointer",
              opacity: !encodeFile || !secretMessage ? 0.5 : 1,
              fontWeight: "bold",
            }}
          >
            Sisipkan Pesan
        ...
        <div className="glass-card animate-fade-in">
          ...
          <button
            ...
            style={{
              ...
              padding: "0.75rem 1.5rem",
              borderRadius: "var(--radius-md)",
              cursor: !decodeFile ? "not-allowed" : "pointer",
              opacity: !decodeFile ? 0.5 : 1,
              fontWeight: "bold",
            }}
          >
            Bongkar Metadata
          ...
          {decodedMessage && (
            ...
            >
              ...
              >
                <code style={{ color: "#fff", whiteSpace: "pre-wrap" }}>
                  {decodedMessage}
                </code>
              </div>
            </div>
